...
from src.utils.config import ADS_CORPUS_DIR, ADS_HELIO_CORPUS_DIR, UATS_RDF_V6, CACHE_DIR, BIBTEX_PATH
...
# Hyperparameters
FROM_HF: bool     = True # Load corpus from HF instead of disk
...
def build_multihot(
    ...
    for i, ann_list in enumerate(annotations):
        if len(ann_list) == 0:
            raise ValueError("No annotation for document.")
        for uri in ann_list:
            if uri in node2idx:
...
n_labels = len(node2idx)


pipeline_path = CACHE_DIR / "featurizer.joblib"
model_path = CACHE_DIR / f"classifier_{INPUT_DIM}.pt"
...
class TfIdf():
    ...
    def train(self):
        if TRAIN_TFIDF or TRAIN_LINEAR_REG:
            # 1. Load corpus
            if not FROM_HF:
                texts  = []
                uats   = []
        ...
        if TRAIN_TFIDF:
            # 2. Learn TF-IDF
            print("Training TF-IDF vectorizer & transforming text to features...")
            ...
            if pipeline_path.exists():
                print(f"Pipeline found in {pipeline_path}, loading...")
                pipeline = joblib.load(pipeline_path)
                print("Pipeline loaded successfully.")
            else:
                raise FileNotFoundError(f"TRAIN_TFIDF is set to False, but checkpoint {pipeline_path} do not exist.")
        ...
        if TRAIN_LINEAR_REG:
            ...
            if model_path.exists():
                print(f"Model found in {model_path}, loading...")
                model = Model(input_dim=INPUT_DIM, n_labels=n_labels)
                model.load_state_dict(torch.load(model_path, map_location=device))
                model.to(device)
                model.eval()
                print("Model loaded successfully.")
            else:
                raise FileNotFoundError(f"TRAIN_LINEAR_REG is set to False, but checkpoint {model_path} do not exist.")
        self.model = model
...
if __name__ == "__main__":
    ...
    tfidf = TfIdf()
    pipeline, model = tfidf.pipeline, tfidf.model
    text_test = []
    uats_test = []
    bibcodes  = []
    ...
    ### Predict on bibtex corpus ###
    text_test = []
    # uats_test = []
    bibcodes  = []
    for document in reader.read_bibtex(path = BIBTEX_PATH):
        text_test.append(document.text)
        # uats_test.append(document.uats)
        bibcodes.append(document.bibcode)
    ...
    print(top_k)
    for idx, scores, bibcode in zip(top_k.indices, top_k.values, bibcodes):
        predicted_uats = [idx2node[int(uat)] for uat in idx]
        predicted_uats_labels = [node2label.get(uat, "UNKNOWN") for uat in predicted_uats]
