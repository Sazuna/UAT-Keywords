...
def get_uat_label(uat_uri: Union[int, str]):
    if type(uat_uri) == int or type(uat_uri) == str and uat_uri.isdigit():
        uat_uri = UAT_NAMESPACE + str(uat_uri)
    if uat_uri not in uat_labels:
        return (uat_uri + " [DEPRECATED]")
